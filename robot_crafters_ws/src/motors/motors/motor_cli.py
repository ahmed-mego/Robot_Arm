...
class AddTwoIntsClientNode(Node):
    ...
    def call_Move_motor_server(self,name, s, d):
        ...
        future.add_done_callback(partial(self.callback_call_move_motor,name=name))

    def callback_call_move_motor(self, future, name):
        try:
            response = future.result()
            self.get_logger().info("the new location of " + name + "  is ---> " + str(response.new_location))
